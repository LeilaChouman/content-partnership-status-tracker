...
import { Plus, Trash2, Download, ChevronDown, ChevronUp, Undo2, Shield, Lock } from 'lucide-react';
import { initializeApp } from 'firebase/app';
import { getFirestore, collection, addDoc, updateDoc, deleteDoc, doc, onSnapshot, query, setDoc, serverTimestamp } from 'firebase/firestore';
import { getAuth, signInWithEmailAndPassword, onAuthStateChanged, signOut, sendEmailVerification } from 'firebase/auth';

export default function SecureTeamTracker() {
  ...
  const handleUndo = async () => {
    ...
    try {
      if (lastAction.type === 'delete_po') {
        ...
        await logAction('UNDO_DELETE_PO', { poId: lastAction.data.id, poNumber: lastAction.data.poNumber });
        ...
        if (po) {
          ...
          await logAction('UNDO_DELETE_CAMPAIGN', { poId: lastAction.poId, campaignName: lastAction.data.name });
        }
      } else if (lastAction.type === 'update') {
        await updateDoc(doc(db, 'shared_pos', lastAction.id), {
          [lastAction.field]: lastAction.oldValue
        });
        await logAction('UNDO_UPDATE', { poId: lastAction.id, field: lastAction.field });
      }
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser && !currentUser.emailVerified) {
        alert('Please verify your email. Check your inbox for verification link.');
        await signOut(auth);
        setUser(null);
      } else {
        setUser(currentUser);
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      if (!result.user.emailVerified) {
        alert('Email not verified. Sending verification email...');
        await sendEmailVerification(result.user);
        await signOut(auth);
        return;
      }
      
      await logAction('USER_LOGIN', { email: email });
      setEmail('');
      setPassword('');
    } catch (error) {
      await logAction('FAILED_LOGIN', { email: email, error: error.code });
      alert(error.message);
    }
  };
  ...
  const handleSignOut = async () => {
    try {
      setUndoStack([]); // Clear sensitive data
  ...
  const addPO = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'shared_pos'), newPO);
      await logAction('CREATE_PO', { poNumber: newPO.poNumber });
    } catch (error) {
  ...
  const deletePO = async (id) => {
    ...
    if (!po) return;
    if (!window.confirm('Delete this PO? You can undo this action.')) return;
    
    try {
      addToUndoStack({ type: 'delete_po', data: po });
      await deleteDoc(doc(db, 'shared_pos', id));
      await logAction('DELETE_PO', { poId: id, poNumber: po.poNumber });
    } catch (error) {
  ...
  const addCampaign = async (poId) => {
    ...
    try {
      ...
      await logAction('ADD_CAMPAIGN', { poId: poId });
    } catch (error) {
  ...
  const deleteCampaign = async (poId, campaignId) => {
    ...
    if (!campaign) return;
    if (!window.confirm('Delete this campaign? You can undo this action.')) return;
    ...
    try {
      ...
      await logAction('DELETE_CAMPAIGN', { poId: poId, campaignName: campaign.name });
    } catch (error) {
  ...
  const exportToCSV = async () => {
    await logAction('EXPORT_DATA', { recordCount: pos.length });
    ...
    a.href = url;
    a.download = `secure_tracker_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-lg shadow-md p-8 w-full max-w-md">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h1 className="text-2xl font-bold text-gray-800">Secure Team Tracker</h1>
          </div>
          <p className="text-sm text-gray-600 mb-6">Enterprise-grade security with email verification</p>
          ...
          <p className="mt-4 text-xs text-gray-500 text-center">Contact admin@omc.com for access</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-3xl font-bold text-gray-800">Secure Team Tracker</h1>
